...
write_fwTime = rospy.Publisher('fw_time', Int32,queue_size=10)
# END PUB
############################################

driving_forward = True
state_change = False # Detects when some motor needs to change its direction of rotation
#First time span for JUS to turn to other direction
##### CONVERT THI INTO A FUNCTION
rnd=random.random()
rndTime=rospy.Duration(fwFactor*cycle*rnd)+rospy.Duration(time_shift)
state_change_time = rospy.Time.now() + rndTime                        
...
while not rospy.is_shutdown():
	...
	state_change = False
        ...
        write_left.publish(speed_L)
        write_right.publish(speed_R)
        write_fwTime.publish(int(rndTime.to_sec()))
